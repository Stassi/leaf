...
const baseLayers = Object.fromEntries(
    ...
    ].map(({ attribution, name, urlTemplate }) => [
      name,
      tileLayer({
        attribution,
        maxZoom: 19,
  ...
  overlays = Object.fromEntries(
    ...
    ].map(({ data, name: overlayName, popupContentTemplate }) => [
      overlayName,
      layerGroup(
        data.map(({ latitudeLongitude, name: placeName }) =>
          marker({
            latitudeLongitude,
...
layers({
  baseLayers,
  map: map({
    activeLayers: [baseLayers.OpenStreetMap, overlays.Cities],
    center: [39.73, -104.99],
    id: 'map',
    zoom: 10,
  }),
