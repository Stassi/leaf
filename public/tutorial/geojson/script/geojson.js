...
leafletGeoJson({
  data,
  filter({ properties: { underConstruction } = {} }) {
    return !underConstruction
  },
  map,
  onEachFeature(
    { properties: { popupContent, underConstruction } = {} },
    layer,
  ) {
  ...
  style({ geometry: { type }, properties: { faction } }) {
    if (type === 'LineString')
      return {
        color: '#ff7800',
        opacity: 0.65,
    ...
    if (faction)
      return {
        color: faction === 'Pluralist' ? '#0000ff' : '#ff0000',
      }
  },
})
