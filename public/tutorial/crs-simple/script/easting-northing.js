...
import { longitudeLatitude as xy } from '../../../leaflet-adapter/longitude-latitude.js'
...
waypoints.forEach(({ longitudeLatitude, name }) => {
  marker({
...
polyline({
  latitudeLongitudes: waypoints
    .filter(({ name }) => ['Deneb', 'Sol'].includes(name))
    .map(({ longitudeLatitude }) => xy(longitudeLatitude)),
  map,
})
